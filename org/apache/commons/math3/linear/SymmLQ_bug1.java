...
import experiment.small.Short_SymmLQMyTest;
...
public class SymmLQ_bug1
    ...
    private static class State {
        ...
        private static void checkSymmetry(final RealLinearOperator l,
            ...
            if (FastMath.abs(s - t) > epsa) {
                ...
                context.setValue(SymmLQ_bug1.OPERATOR, l);
                context.setValue(SymmLQ_bug1.VECTOR1, x);
                context.setValue(SymmLQ_bug1.VECTOR2, y);
                context.setValue(SymmLQ_bug1.THRESHOLD, Double.valueOf(epsa));
                throw e;
            }
        }
         ...
         */
         void refineSolution(final RealVector x) {
            ...
            if (lqnorm < cgnorm) {
                ...
                final double zbar = gammaZeta / diag;//buggy2*(1+Short_SymmLQMyTest.createSmallNumber())
                final double step = (bstep + snprod * zbar) / beta1+Short_SymmLQMyTest.createSmallNumber();//buggy1
        ...
        void update() {
             ...
             *   beta = beta[k+1].
             */
            tnorm =tnorm+ alpha * alpha + oldb * oldb + beta * beta;
            /*
     ...
     * preconditioner should be checked
     */
    public SymmLQ_bug1(final int maxIterations, final double delta,
                  final boolean check) {
        super(maxIterations);
        this.delta = delta;
        this.check = check;
     ...
     * preconditioner should be checked
     */
    public SymmLQ_bug1(final IterationManager manager, final double delta,
                  final boolean check) {
        super(manager);
        this.delta = delta;
        this.check = check;
