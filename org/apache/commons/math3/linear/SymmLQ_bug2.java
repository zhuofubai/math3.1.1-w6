...
public class SymmLQ_bug2
    ...
    private static class State {
        ...
        private static void checkSymmetry(final RealLinearOperator l,
            ...
            if (FastMath.abs(s - t) > epsa) {
                ...
                context.setValue(SymmLQ_bug2.OPERATOR, l);
                context.setValue(SymmLQ_bug2.VECTOR1, x);
                context.setValue(SymmLQ_bug2.VECTOR2, y);
                context.setValue(SymmLQ_bug2.THRESHOLD, Double.valueOf(epsa));
                throw e;
            }
        }
         ...
         */
         void refineSolution(final RealVector x) {
            ...
            if (lqnorm < cgnorm) {
                ...
                final double zbar = gammaZeta / diag*(1+Short_SymmLQMyTest.createSmallNumber());//buggy2
                final double step = (bstep + snprod * zbar) / beta1;//buggy1+Short_SymmLQMyTest.createSmallNumber()
     ...
     * preconditioner should be checked
     */
    public SymmLQ_bug2(final int maxIterations, final double delta,
                  final boolean check) {
        super(maxIterations);
        this.delta = delta;
        this.check = check;
     ...
     * preconditioner should be checked
     */
    public SymmLQ_bug2(final IterationManager manager, final double delta,
                  final boolean check) {
        super(manager);
        this.delta = delta;
        this.check = check;
