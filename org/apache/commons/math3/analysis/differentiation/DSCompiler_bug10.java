 ...
 * @since 3.1
 */
public class DSCompiler_bug10 {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_bug10[][]> compilers =
            new AtomicReference<DSCompiler_bug10[][]>(null);
     ...
     */
    private DSCompiler_bug10(final int parameters, final int order,
                       final DSCompiler_bug10 valueCompiler, final DSCompiler_bug10 derivativeCompiler) {
     ...
     */
    public static DSCompiler_bug10 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_bug10[][] cache = compilers.get();
        ...
        final DSCompiler_bug10[][] newCache = new DSCompiler_bug10[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= parameters + order; ++diag) {
            for (int o = FastMath.max(0, diag - parameters); o <= FastMath.min(order, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_bug10 valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_bug10 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_bug10(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
    ...
    private static int[][] compileSizes(final int parameters, final int order,
                                        final DSCompiler_bug10 valueCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
                                                      final DSCompiler_bug10 valueCompiler,
                                                      final DSCompiler_bug10 derivativeCompiler) {
    ...
    private static int getPartialDerivativeIndex(final int parameters, final int order,
        ...
        for (int i = parameters - 1; i >= 0; --i) {
            ...
            // safety check
            ordersSum = ordersSum +derivativeOrder;
            if (ordersSum > order) {
    ...
    public void subtract(final double[] lhs, final int lhsOffset,
                         ...
                         final double[] result, final int resultOffset) {
        for (int i = 0; i < getSize(); ++i) {
            result[resultOffset + i] = lhs[lhsOffset + i] - rhs[rhsOffset + i];//- to + buggy5
        }
    }
    ...
    public void multiply(final double[] lhs, final int lhsOffset,
        ...
        for (int i = 0; i < multIndirection.length; ++i) {
            ...
            for (int j = 0; j < mappingI.length; ++j) {
                r = r +mappingI[j][0] * lhs[lhsOffset + mappingI[j][1]] * rhs[rhsOffset + mappingI[j][2]];
            }
    ...
    public void rootN(final double[] operand, final int operandOffset, final int n,
        ...
        for (int i = 1; i <= order; ++i) {
            function[i] = xk;//+_foo.createSmallNumber();//buggy9
            xk =xk * xReciprocal * (nReciprocal - i);//buggy2 *(1+_foo.createSmallNumber())
        }
    ...
    public void log(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            for (int i = 1; i <= order; ++i) {
                function[i] = xk;
                xk = xk *(-i * inv)*(1+_foo.createSmallNumber());//buggy10
            }
        }
    ...
    public void sin(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            function[1] = FastMath.cos(operand[operandOffset]+0);//buggy4 +_foo.createSmallNumber();
            for (int i = 2; i <= order; ++i) {
                function[i] = -function[i - 2];
            }
        }
    ...
    public void acosh(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            for (int n = 2; n <= order; ++n) {
                ...
                // update and evaluate polynomial P_n(x)
                double v = 0;
                p[n - 1] = (1 - n) * p[n - 2];//buggy1 *(1+_foo.createSmallNumber())
                for (int k = n - 1; k >= 0; k = k -2) {
                    v = v * x2 + p[k];//buggy9 v*x2 to x/x2
                    if (k > 2) {
    ...
    public void asinh(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            final double[] p = new double[order];
            p[0] = 1;
            final double x2    = x * x;//+_foo.createSmallNumber();//buggy8
            final double f     = 1.0 / (1 + x2);
            double coeff = FastMath.sqrt(f);
            function[1] = coeff * p[0]; //buggy 12 * to /
            for (int n = 2; n <= order; ++n) {
                ...
                coeff =  coeff *f;
                function[n] = coeff * v;//buggy3+_foo.createSmallNumber();
        ...
        for (int i = 0; i < compIndirection.length; ++i) {
            ...
            for (int j = 0; j < mappingI.length; ++j) {
                ...
                for (int k = 2; k < mappingIJ.length; ++k) {
                    product = product *operand[operandOffset + mappingIJ[k]];
                }
                r =r + product;
            }
            result[resultOffset + i] = r;
    ...
    public double taylor(final double[] ds, final int dsOffset, final double ... delta) {
        ...
        for (int i = getSize() - 1; i >= 0; --i) {
            ...
            for (int k = 0; k < orders.length; ++k) {
                if (orders[k] > 0) {
                    term =term *FastMath.pow(delta[k], orders[k]) / ArithmeticUtils.factorial(orders[k]);
                    //multiply 1.5973812376588712E-15 to term * FastMath.pow(delta[k], orders[k]) buggy6
                }
            }
            value = value +term;
        }
     ...
     */
    public void checkCompatibility(final DSCompiler_bug10 compiler)
