 ...
 * @since 3.1
 */
public class DSCompiler_bug7 {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_bug7[][]> compilers =
            new AtomicReference<DSCompiler_bug7[][]>(null);
     ...
     */
    private DSCompiler_bug7(final int parameters, final int order,
                       final DSCompiler_bug7 valueCompiler, final DSCompiler_bug7 derivativeCompiler) {
     ...
     */
    public static DSCompiler_bug7 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_bug7[][] cache = compilers.get();
        ...
        final DSCompiler_bug7[][] newCache = new DSCompiler_bug7[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= parameters + order; ++diag) {
            for (int o = FastMath.max(0, diag - parameters); o <= FastMath.min(order, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_bug7 valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_bug7 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_bug7(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
    ...
    private static int[][] compileSizes(final int parameters, final int order,
                                        final DSCompiler_bug7 valueCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
                                                      final DSCompiler_bug7 valueCompiler,
                                                      final DSCompiler_bug7 derivativeCompiler) {
     ...
     */
    public void checkCompatibility(final DSCompiler_bug7 compiler)
