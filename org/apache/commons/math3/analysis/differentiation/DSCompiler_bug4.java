 ...
 * @since 3.1
 */
public class DSCompiler_bug4 {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_bug4[][]> compilers =
            new AtomicReference<DSCompiler_bug4[][]>(null);
     ...
     */
    private DSCompiler_bug4(final int parameters, final int order,
                       final DSCompiler_bug4 valueCompiler, final DSCompiler_bug4 derivativeCompiler) {
     ...
     */
    public static DSCompiler_bug4 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_bug4[][] cache = compilers.get();
        ...
        final DSCompiler_bug4[][] newCache = new DSCompiler_bug4[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= parameters + order; ++diag) {
            for (int o = FastMath.max(0, diag - parameters); o <= FastMath.min(order, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_bug4 valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_bug4 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_bug4(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
    ...
    private static int[][] compileSizes(final int parameters, final int order,
                                        final DSCompiler_bug4 valueCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
                                                      final DSCompiler_bug4 valueCompiler,
                                                      final DSCompiler_bug4 derivativeCompiler) {
    ...
    public void sin(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            function[1] = FastMath.cos(operand[operandOffset]+0)+_foo.createSmallNumber();//buggy4
            for (int i = 2; i <= order; ++i) {
                function[i] = -function[i - 2];
            }
        }
    ...
    public void atan2(final double[] y, final int yOffset,
        ...
        if (x[xOffset] >= 0) {
            ...
            if(tmp2[0] <= 0){
            	...
            	result[resultOffset] = FastMath.PI - 2 * tmp2[0];// buggy5 x*tmp2[0] to x/tmp2[0]
            }
    ...
    public void acosh(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            for (int n = 2; n <= order; ++n) {
                ...
                for (int k = n - 1; k >= 0; k = k -2) {
                    v = v * x2 + p[k];//buggy 4 v*x2 to x/x2
                    if (k > 2) {
     ...
     */
    public void checkCompatibility(final DSCompiler_bug4 compiler)
