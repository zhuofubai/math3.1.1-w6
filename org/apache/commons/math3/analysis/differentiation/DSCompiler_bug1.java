 ...
 * @since 3.1
 */
public class DSCompiler_bug1 {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_bug1[][]> compilers =
            new AtomicReference<DSCompiler_bug1[][]>(null);
     ...
     */
    private DSCompiler_bug1(final int parameters, final int order,
                       final DSCompiler_bug1 valueCompiler, final DSCompiler_bug1 derivativeCompiler) {
     ...
     */
    public static DSCompiler_bug1 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_bug1[][] cache = compilers.get();
        ...
        final DSCompiler_bug1[][] newCache = new DSCompiler_bug1[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= parameters + order; ++diag) {
            for (int o = FastMath.max(0, diag - parameters); o <= FastMath.min(order, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_bug1 valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_bug1 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_bug1(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
    ...
    private static int[][] compileSizes(final int parameters, final int order,
                                        final DSCompiler_bug1 valueCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
                                                      final DSCompiler_bug1 valueCompiler,
                                                      final DSCompiler_bug1 derivativeCompiler) {
    ...
    public void acosh(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            for (int n = 2; n <= order; ++n) {
                ...
                // update and evaluate polynomial P_n(x)
                double v = 0;
                p[n - 1] = (1 - n) * p[n - 2]*(1+_foo.createSmallNumber());//bug1
                for (int k = n - 1; k >= 0; k = k -2) {
                    v = v * x2 + p[k];
                    if (k > 2) {
     ...
     */
    public void checkCompatibility(final DSCompiler_bug1 compiler)
