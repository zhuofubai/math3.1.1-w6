 ...
 * @since 3.1
 */
public class DSCompiler_bug2 {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_bug2[][]> compilers =
            new AtomicReference<DSCompiler_bug2[][]>(null);
     ...
     */
    private DSCompiler_bug2(final int parameters, final int order,
                       final DSCompiler_bug2 valueCompiler, final DSCompiler_bug2 derivativeCompiler) {
     ...
     */
    public static DSCompiler_bug2 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_bug2[][] cache = compilers.get();
        ...
        final DSCompiler_bug2[][] newCache = new DSCompiler_bug2[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= parameters + order; ++diag) {
            for (int o = FastMath.max(0, diag - parameters); o <= FastMath.min(order, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_bug2 valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_bug2 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_bug2(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
    ...
    private static int[][] compileSizes(final int parameters, final int order,
                                        final DSCompiler_bug2 valueCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
                                                      final DSCompiler_bug2 valueCompiler,
                                                      final DSCompiler_bug2 derivativeCompiler) {
    ...
    public void rootN(final double[] operand, final int operandOffset, final int n,
        ...
        for (int i = 1; i <= order; ++i) {
            function[i] = xk;
            xk =xk * xReciprocal * (nReciprocal - i)*(1+_foo.createSmallNumber());//buggy2
        }
     ...
     */
    public void checkCompatibility(final DSCompiler_bug2 compiler)
