 ...
 * @since 3.1
 */
public class DSCompiler_bug_tem {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
//    private static AtomicReference<DSCompiler_bug[][]> compilers =
//            new AtomicReference<DSCompiler_bug[][]>(null);
     ...
     */
    private DSCompiler_bug_tem(final int parameters, final int order,
    ...
    public DSCompiler_bug_tem( int parameters,  int order,int[][] sizes, int[][]derivativesIndirection,int[] lowerIndirection,int [][][] multIndirection,int [][][] compIndirection){
    	 this.parameters = parameters;
         this.order      = order;
         this.sizes      = sizes;
     ...
     */
//    public static DSCompiler_bug getCompiler(int parameters, int order) {
//
//        // get the cached compilers
//        final DSCompiler_bug[][] cache = compilers.get();
...
//        final DSCompiler_bug[][] newCache = new DSCompiler_bug[maxParameters + 1][maxOrder + 1];
//
//        if (cache != null) {
...
//                    final DSCompiler_bug valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
//                    final DSCompiler_bug derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
//                    newCache[p][o] = new DSCompiler_bug(p, o, valueCompiler, derivativeCompiler);
//                }
//            }
//        }
//
...
//    }
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
        ...
        for (int i = 0; i < vSize; ++i) {
            ...
            System.arraycopy(valueCompiler.getderivativesIndirection()[i], 0,
        ...
        for (int i = 0; i < dSize; ++i) {
            ...
            // copy the indices
            System.arraycopy(derivativeCompiler.getderivativesIndirection()[i], 0,
    ...
    private static int[][][] compileCompositionIndirection(final int parameters, final int order,
        ...
        for (int i = 0; i < dSize; ++i) {
            List<int[]> row = new ArrayList<int[]>();
            for (int[] term : derivativeCompiler.getcompIndirection()[i]) {
                ...
                for (int j = 2; j < term.length; ++j) {
                    ...
                    derivedTermF[j] = convertIndex(term[j], parameters,
                                                   derivativeCompiler.getderivativesIndirection(),
    ...
    public void rootN(final double[] operand, final int operandOffset, final int n,
        ...
        for (int i = 1; i <= order; ++i) {
            function[i] = xk;
            xk =xk * xReciprocal * (nReciprocal - i);
        }
    ...
    public void atan(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            final double f     = 1.0 / (1 + x2);
            double coeff = f;
            function[1] = coeff * q[0];//+_foo.createSmallNumber();
            for (int n = 2; n <= order; ++n) {
     ...
     */
    public void checkCompatibility(final DSCompiler_bug_tem compiler)
