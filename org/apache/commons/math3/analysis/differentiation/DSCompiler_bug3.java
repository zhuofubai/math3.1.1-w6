 ...
 * @since 3.1
 */
public class DSCompiler_bug3 {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_bug3[][]> compilers =
            new AtomicReference<DSCompiler_bug3[][]>(null);
     ...
     */
    private DSCompiler_bug3(final int parameters, final int order,
                       final DSCompiler_bug3 valueCompiler, final DSCompiler_bug3 derivativeCompiler) {
     ...
     */
    public static DSCompiler_bug3 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_bug3[][] cache = compilers.get();
        ...
        final DSCompiler_bug3[][] newCache = new DSCompiler_bug3[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= parameters + order; ++diag) {
            for (int o = FastMath.max(0, diag - parameters); o <= FastMath.min(order, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_bug3 valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_bug3 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_bug3(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
    ...
    private static int[][] compileSizes(final int parameters, final int order,
                                        final DSCompiler_bug3 valueCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
                                                      final DSCompiler_bug3 valueCompiler,
                                                      final DSCompiler_bug3 derivativeCompiler) {
    ...
    public void pow(final double[] operand, final int operandOffset, final double p,
        ...
        for (int i = 1; i <= order; ++i) {
            ...
            coefficient =coefficient *( p - i);//buggy4 *_foo.createsmallnumber()
        }
    ...
    public void asinh(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            for (int n = 2; n <= order; ++n) {
                ...
                coeff =  coeff *f;
                function[n] = coeff * v+_foo.createSmallNumber();//buggy3
     ...
     */
    public void checkCompatibility(final DSCompiler_bug3 compiler)
