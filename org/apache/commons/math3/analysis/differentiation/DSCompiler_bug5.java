 ...
 * @since 3.1
 */
public class DSCompiler_bug5 {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_bug5[][]> compilers =
            new AtomicReference<DSCompiler_bug5[][]>(null);
     ...
     */
    private DSCompiler_bug5(final int parameters, final int order,
                       final DSCompiler_bug5 valueCompiler, final DSCompiler_bug5 derivativeCompiler) {
     ...
     */
    public static DSCompiler_bug5 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_bug5[][] cache = compilers.get();
        ...
        final DSCompiler_bug5[][] newCache = new DSCompiler_bug5[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= parameters + order; ++diag) {
            for (int o = FastMath.max(0, diag - parameters); o <= FastMath.min(order, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_bug5 valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_bug5 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_bug5(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
    ...
    private static int[][] compileSizes(final int parameters, final int order,
                                        final DSCompiler_bug5 valueCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
                                                      final DSCompiler_bug5 valueCompiler,
                                                      final DSCompiler_bug5 derivativeCompiler) {
    ...
    public void sin(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            function[1] = FastMath.cos(operand[operandOffset]+0);//buggy4
            for (int i = 2; i <= order; ++i) {
                function[i] = -function[i - 2];
            }
        }
     ...
     */
    public void checkCompatibility(final DSCompiler_bug5 compiler)
