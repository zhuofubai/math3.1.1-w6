 ...
 * @since 3.1
 */
public class DSCompiler_bug6 {
	Foo _foo=new Foo();
    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_bug6[][]> compilers =
            new AtomicReference<DSCompiler_bug6[][]>(null);
     ...
     */
    private DSCompiler_bug6(final int parameters, final int order,
                       final DSCompiler_bug6 valueCompiler, final DSCompiler_bug6 derivativeCompiler) {
     ...
     */
    public static DSCompiler_bug6 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_bug6[][] cache = compilers.get();
        ...
        final DSCompiler_bug6[][] newCache = new DSCompiler_bug6[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= parameters + order; ++diag) {
            for (int o = FastMath.max(0, diag - parameters); o <= FastMath.min(order, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_bug6 valueCompiler      = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_bug6 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_bug6(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
    ...
    private static int[][] compileSizes(final int parameters, final int order,
                                        final DSCompiler_bug6 valueCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
                                                      final DSCompiler_bug6 valueCompiler,
                                                      final DSCompiler_bug6 derivativeCompiler) {
    ...
    public void acosh(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            for (int n = 2; n <= order; ++n) {
                ...
                for (int k = n - 1; k >= 0; k = k -2) {
                    v = v * x2 + p[k];//buggy8 v*x2 to x/x2
                    if (k > 2) {
    ...
    public void asinh(final double[] operand, final int operandOffset,
        ...
        if (order > 0) {
            ...
            final double f     = 1.0 / (1 + x2);
            double coeff = FastMath.sqrt(f);
            function[1] = coeff * p[0];//buggy7 * to /
            for (int n = 2; n <= order; ++n) {
    ...
    public double taylor(final double[] ds, final int dsOffset, final double ... delta) {
        ...
        for (int i = getSize() - 1; i >= 0; --i) {
            ...
            for (int k = 0; k < orders.length; ++k) {
                if (orders[k] > 0) {
                    term =term *(1+10*_foo.createSmallNumber()) *FastMath.pow(delta[k], orders[k]) / ArithmeticUtils.factorial(orders[k]);
     ...
     */
    public void checkCompatibility(final DSCompiler_bug6 compiler)
