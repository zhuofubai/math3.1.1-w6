...
import experiment.small.Short_TestFastCosine;

//import experiment.TestFastCosine;
 ...
 * @since 1.2
 */
public class FastCosineTransformer_bug1 implements RealTransformer, Serializable {
     ...
     * transformed data
     */
    public FastCosineTransformer_bug1(final DctNormalization normalization) {
    ...
    protected double[] fct(double[] f)
        ...
        double t1 = 0.5 * (f[0] - f[n])*(1+Short_TestFastCosine.createSmallNumber());//buggy1
        for (int i = 1; i < (n >> 1); i++) {
        ...
        FastFourierTransformer_bug transformer;
        transformer = new FastFourierTransformer_bug(DftNormalization.STANDARD);
        ...
        for (int i = 1; i < (n >> 1); i++) {
            ...
            double temp=y[i].getImaginary();
            transformed[2 * i + 1] = transformed[2 * i - 1] - temp;
        }
