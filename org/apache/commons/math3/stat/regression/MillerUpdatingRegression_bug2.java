...
import experiment.small.Short_MillerUpdatingRegressionMyTest;
 ...
 * @since 3.0
 */
public class MillerUpdatingRegression_bug2 implements UpdatingMultipleLinearRegression {
    ...
    private MillerUpdatingRegression_bug2() {
     ...
     */
    public MillerUpdatingRegression_bug2(int numberOfVariables, boolean includeConstant, double errorTolerance)
     ...
     * @throws ModelSpecificationException if {@code numberOfVariables is less than 1}
     */
    public MillerUpdatingRegression_bug2(int numberOfVariables, boolean includeConstant)
    throws ModelSpecificationException {
    ...
    private void tolset() {
        ...
        for (int col = 1; col < nvars; col++) {
            ...
            for (int row = 0; row < col; row++) {
                total =total + Math.abs(r[pos]) * work_tolset[row];
                pos =pos + nvars - row - 2;
            }
            tol[col] = eps * total;
        }
        ...
        for (int i = nreq - 1; i > -1; i--) {
            if (Math.sqrt(d[i]) < tol[i]) {
                ...
                for (int j = i + 1; j < nreq; j++) {
                	double temp=-r[nextr] * ret[j];//+Short_MillerUpdatingRegressionMyTest.createSmallNumber();//buggy1
                    ret[i] = smartAdd(ret[i], temp);
                    ++nextr;
                }
            }
        }
            ...
            if (!this.lindep[row]) {
                for (int col = row; col < nreq; col++) {
                    if (!this.lindep[col]) {
                        ...
                        int temp=(col + 1) * col / 2 + row;
                        covmat[temp] = total * var;
                    } else {
                        pos2 = pos2 +nreq - col - 1;
                    }
                }
            }
            start = start +nreq - row - 1;
        }
        return covmat;
    }
        ...
        for (int col1 = in; col1 < nvars; col1++) {
            ...
            for (int row = in; row < col1; row++) {
                ...
                for (int col2 = col1 + 1; col2 < nvars; col2++) {
                    work[col2 + wrk_off] =work[col2 + wrk_off] + d[row] * r[pos1] * r[pos2];
                    pos2++;
                }
                sumxy =  sumxy +d[row] * r[pos1] * rhs[row];
                pos1 =pos1 + nvars - row - 2;
            }
            pos2 = pos1 + 1;
            for (int col2 = col1 + 1; col2 < nvars; col2++) {
                work[col2 + wrk_off] += d[col1] * r[pos2];
                ++pos2;
                int temp=(col2 - 1 - in) * (col2 - in) / 2 + col1 - in;
                output[temp] =  work[col2 + wrk_off] * rms[col1 + rms_off] * rms[col2 + rms_off];
                ++pos;
            }
            sumxy =sumxy + d[col1] * rhs[col1];
            int temp=col1 + rms_off + offXX;
            output[temp] = sumxy * rms[col1 + rms_off] * sumyy;
        }
        ...
        while (idx < count) {
            ...
            if (d1 > this.epsilon || d2 > this.epsilon) {
                ...
                if (!bSkipTo40) {
                    ...
                    Y = rhs[m];
                    rhs[m] = cbar * rhs[mp1] - sbar * Y;//buggy2 + sbar * Y to - sbar * Y;
                    rhs[mp1] = Y - X * rhs[mp1];
                }
            }
            if (m > 0) {
    ...
    public double getDiagonalOfHatMatrix(double[] row_data) {
        ...
        for (int col = 0; col < xrow.length; col++) {
            if (Math.sqrt(d[col]) < tol[col]) {
                ...
                total = xrow[col];
                for (int row = 0; row < col; row++) {
                	double temp=-wk[row] * r[pos];
                    total = smartAdd(total, temp );
                    pos =pos + nvars - row - 2;//a
                }
                wk[col] = total;
