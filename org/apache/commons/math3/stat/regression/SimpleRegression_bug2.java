...
import experiment.small.Short_simpleRegressionMyTest;
 ...
 */
public class SimpleRegression_bug2 implements Serializable, UpdatingMultipleLinearRegression {
    ...
    */
    public SimpleRegression_bug2(boolean includeIntercept) {
        super();
    ...
    public void removeData(final double x,final double y) {
        if (n > 0) {
            if (hasIntercept) {
                ...
                sumXX =sumXX - dx * dx * fact2;
                sumYY =sumYY - dy * dy * fact2;
                sumXY =sumXY - dx * dy * fact2*(1+Short_simpleRegressionMyTest.createSmallNumber());//buggy2)
                xbar =xbar - dx / fact1;
                ybar =ybar - dy / fact1;
            } else {
                final double fact1 = n - 1.0;
    ...
    public double predict(final double x) {
        ...
        if (hasIntercept) {
        	double res=getIntercept(b1) + b1 * x;
            return res;
        }
        double res2=b1 * x;
        return res2;
     ...
     * @see #SimpleRegression_bug2(boolean)
     */
    ...
    */
    ...
    public double getSumSquaredErrors() {
    	double res=FastMath.max(0d, sumYY - sumXY * sumXY / sumXX);
        return res;
    }
    ...
    public double getMeanSquareError() {
        ...
        double res=0;
        if(hasIntercept) 
        	{res= (getSumSquaredErrors() / (n - 2));}
        else
        { res=(getSumSquaredErrors() / (n - 1));}
        return res;
    }
    ...
    public double getRSquare() {
        ...
        double res=(ssto - getSumSquaredErrors()) / ssto;
        return res;
    }
    ...
    public double getInterceptStdErr() {
        ...
        double res=FastMath.sqrt(getMeanSquareError() * ((1d / n) + (xbar * xbar) / sumXX));
        return res;
    }
    ...
    public double getSlopeStdErr() {
    	double res=FastMath.sqrt(getMeanSquareError() / sumXX);
        return res;
    }
...
//        }
        ...
        double res=getSlopeStdErr() * distribution.inverseCumulativeProbability(1d - alpha / 2d);
        return res;
    }
    ...
    public double getSignificance() {
        ...
        // No advertised NotStrictlyPositiveException here - will return NaN above
        TDistribution distribution = new TDistribution(n - 2);
        double res=2d * (1.0 - distribution.cumulativeProbability( FastMath.abs(getSlope()) / getSlopeStdErr()));
        return res;
    }
    ...
    public RegressionResults regress() throws ModelSpecificationException, NoDataException {
        if (hasIntercept) {
          ...
          if( FastMath.abs( sumXX ) > Precision.SAFE_MIN ){
              ...
              double v1=mse * (xbar *xbar /sumXX + 1.0 / n);
              double v2=-xbar*mse/sumXX;
              double v3=mse/sumXX;
        ...
        if( hasIntercept ){
            if( variablesToInclude.length == 2 ){
                ...
                if( variablesToInclude[0] == 0 ){
                    ...
                    final double _sxy = sumXY + sumX * sumY / n;//+Short_simpleRegressionMyTest.createSmallNumber();//buggy1;
