...
import experiment.small.Short_ChiSquareTestMyTest;
 ...
 */
public class ChiSquareTest_bug3 {

    /**
     * Construct a ChiSquareTest
     */
    public ChiSquareTest_bug3() {
        super();
    }
...
//        double sumObserved = 0d;
        ...
        for (int i = 0; i < observed.length; i++) {
        	 dev = ((double) observed[i] - expected[i]);
  	 	 	 sumSq =sumSq + dev * dev / expected[i];
//            sumExpected = sumExpected + expected[i];
//            sumObserved =sumObserved + observed[i];
        }
//        double ratio = 1.0d;
...
//        for (int i = 0; i < observed.length; i++) {
//            if (rescale) {
//                final double dev = observed[i]*(1+Short_ChiSquareTestMyTest.createSmallNumber()) - ratio * expected[i];
//                sumSq = sumSq +dev * dev / (ratio * expected[i]);
//            } else {
...
//        }
    ...
    public double chiSquare(final long[][] counts)
        ...
        for (int row = 0; row < nRows; row++) {
            for (int col = 0; col < nCols; col++) {
                rowSum[row] =rowSum[row] + counts[row][col];
                colSum[col] =colSum[col] + counts[row][col];
                total =total + counts[row][col];
            }
        }
        ...
        for (int row = 0; row < nRows; row++) {
            for (int col = 0; col < nCols; col++) {
                ...
                sumSq =sumSq + ((counts[row][col] - expected) * (counts[row][col] - expected)) / expected;//
            }
        }
        return sumSq;
    ...
    public double chiSquareDataSetsComparison(long[] observed1, long[] observed2)
        ...
        for (int i = 0; i < observed1.length; i++) {
            countSum1 =countSum1 + observed1[i];
            countSum2 =countSum2 + observed2[i];
        }
        ...
        for (int i = 0; i < observed1.length; i++) {
            if (observed1[i] == 0 && observed2[i] == 0) {
                ...
                sumSq =sumSq + (dev * dev) / (obs1 + obs2);//buggy1 sumsq+ to sumsq -
            }
        }
        return sumSq;
    }
    ...
    public double chiSquareTestDataSetsComparison(long[] observed1, long[] observed2)
        ...
        double m=distribution.cumulativeProbability(
